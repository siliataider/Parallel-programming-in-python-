...
def un_cheval(ma_ligne : int, L,position) : # ma_ligne commence a 0
...
def arbitre(position):
    ...
    while keep_running.value:
        ...
        move_to(22, 1)
        erase_line_from_beg_to_curs()
        print("Best: %c en ligne %2i et position %2i" %(chr(chevalWinnerTemp),ligneWinnerTemp,max))
        print("WInner: %c en ligne %2i et position %2i" %(chr(chevalWinner),ligneWinner,max))
...
if __name__ == "__main__" :
    ...
    signal.signal(signal.SIGINT,fonction_effacer)
    ...
    position=mp.Array("i",[0 for i in range(Nb_process)])
    ...
    for i in range(Nb_process):  # Lancer     Nb_process  processus
        mes_process[i] = Process(target=un_cheval, args= (i,L,position,))
        mes_process[i].start()

    move_to(Nb_process+10, 1)
    print("tous lances")

    P_arbitre=Process(target=arbitre, args=(position,))
